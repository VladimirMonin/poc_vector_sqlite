# 🏗️ Архитектура POC: Семантический поиск на SQLite

> Добро пожаловать в документацию проекта! Здесь объясняется, как работает векторный поиск от начала до конца.

---

## 📖 Оглавление

### 🎓 Основы
1. [**Что такое эмбеддинги?**](doc/architecture/01_embeddings_basics.md)  
   Векторные представления текста и почему они работают

2. [**Gemini API для эмбеддингов**](doc/architecture/02_gemini_api.md)  
   Модели, лимиты, task types и MRL

3. [**SQLite-Vec: хранение векторов**](doc/architecture/03_sqlite_vec.md)  
   Как расширение sqlite-vec работает с BLOB

---

### 🔍 Поиск
4. [**Типы поиска**](doc/architecture/04_search_types.md)  
   Векторный, полнотекстовый и их отличия

5. [**Гибридный поиск (RRF)**](doc/architecture/05_hybrid_search_rrf.md)  
   Reciprocal Rank Fusion — лучшее из двух миров

---

### 🏛️ Архитектура проекта
6. [**Структура проекта**](doc/architecture/06_project_architecture.md)  
   Разделение на semantic_core и domain

7. [**Поток данных: от текста до результата**](doc/architecture/07_data_flow.md)  
   Полный цикл: добавление → индексация → поиск

---

## 🚀 Быстрый старт

```bash
# Установка зависимостей
poetry install

# Настройка .env
cp .env.example .env
# Добавь свой GEMINI_API_KEY

# Запуск тестов
poetry run python main.py
```

---

## 📊 Ключевые концепции

```mermaid
graph LR
    A[Текст заметки] --> B[Gemini API]
    B --> C[Вектор 768D]
    C --> D[SQLite-Vec]
    
    E[Поисковый запрос] --> B
    B --> F[Вектор запроса]
    F --> G[Косинусное сходство]
    D --> G
    G --> H[Результаты]
```

---

## 💡 Зачем это нужно?

Обычный поиск по ключевым словам **не понимает смысл**:
- Запрос "как написать цикл" **не найдёт** заметку про "for loop"
- Запрос "рецепт борща" **не найдёт** заметку про "украинский суп"

**Семантический поиск** решает эту проблему, находя по **смыслу**, а не по буквам!

---

## 🎯 Начни отсюда

1. Сначала прочитай [**Что такое эмбеддинги**](doc/architecture/01_embeddings_basics.md)
2. Потом изучи [**Как работает Gemini API**](doc/architecture/02_gemini_api.md)
3. Дальше по порядку из оглавления ⬆️

Приятного изучения! 🚀
