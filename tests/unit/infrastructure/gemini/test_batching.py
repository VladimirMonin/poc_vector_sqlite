...
class TestEmbeddingExtraction:
    """Тесты извлечения embedding values из ответа."""

    def test_extract_from_embeddings_array(self, mock_embedder):
        """Извлечение из response.embeddings[0].values."""
        from semantic_core.infrastructure.gemini.batching import (
            GeminiBatchClient,
            GENAI_SDK_AVAILABLE,
        )
        ...
        with patch("semantic_core.infrastructure.gemini.batching.genai") as mock_genai:
            ...
            # Эмулируем структуру response.response.embeddings[0].values
            mock_response = MagicMock()
            mock_embedding = MagicMock()
            mock_embedding.values = [0.1, 0.2, 0.3]
            mock_response.response.embeddings = [mock_embedding]

            values = client._extract_embedding_values(mock_response)

            assert values == [0.1, 0.2, 0.3]

    def test_extract_from_embedding_singular(self, mock_embedder):
        """Извлечение из response.embedding.values."""
        from semantic_core.infrastructure.gemini.batching import (
            GeminiBatchClient,
            GENAI_SDK_AVAILABLE,
        )
        ...
        with patch("semantic_core.infrastructure.gemini.batching.genai") as mock_genai:
            ...
            # Эмулируем структуру response.response.embedding.values
            mock_response = MagicMock()
            mock_response.response.embeddings = None
            mock_response.response.embedding.values = [0.4, 0.5, 0.6]

            values = client._extract_embedding_values(mock_response)

            assert values == [0.4, 0.5, 0.6]

    def test_extract_returns_none_on_missing_data(self, mock_embedder):
