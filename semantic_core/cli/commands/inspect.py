...
def inspect(
    ...
    try:
        ...
        console.print(f"[green]✓[/green] Duration: {snapshot.total_duration_ms:.2f}ms\n")
        
        # === STEP 2: Search Inspection (if requested) ===
        if search:
            console.print(f"[bold cyan]STEP 2:[/bold cyan] Search inspection (query: '{search}')...\n")
            ...
            console.print(f"[green]✓[/green] Found {search_snapshot.searches[0].results_count if search_snapshot.searches else 0} results")
            console.print(f"[green]✓[/green] Search time: {search_snapshot.searches[0].search_time_ms if search_snapshot.searches else 0:.2f}ms\n")
        ...
        if format == "console":
            ...
            if len(snapshot.chunks) > 5 and not verbose:
                console.print(f"[dim]... и ещё {len(snapshot.chunks) - 5} chunks (используйте --verbose для полного вывода)[/dim]\n")
            ...
            if save:
                report_path = artifacts_root / f"report_{snapshot.processing_timestamp.strftime('%Y%m%d_%H%M%S')}.md"
                reporter.save_to_file(snapshot, report_path)
                console.print(f"[green]✓[/green] Markdown report saved: {report_path}\n")
            else:
            ...
            if save:
                ...
                console.print(f"[green]✓[/green] JSON snapshot saved: {snapshot_path}\n")
            else:
        ...
        if save and format != "json":  # JSON already saves in reporter
            ...
            if snapshot.searches:
                ...
                console.print(f"[green]✓[/green] Similarity matrix: {similarity_csv_path}")
