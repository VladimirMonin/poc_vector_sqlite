...
class BatchManager:
    ...
    def get_queue_stats(self) -> Dict[str, int]:
        ...
        """
        return {
            "pending": ChunkModel.select().where(
                ChunkModel.embedding_status == EmbeddingStatus.PENDING.value
            ).count(),
            "processing": ChunkModel.select().where(
                (ChunkModel.embedding_status == EmbeddingStatus.PENDING.value) &
                (ChunkModel.batch_job.is_null(False))
            ).count(),
            "ready": ChunkModel.select().where(
                ChunkModel.embedding_status == EmbeddingStatus.READY.value
            ).count(),
            "failed": ChunkModel.select().where(
                ChunkModel.embedding_status == EmbeddingStatus.FAILED.value
            ).count(),
        }
