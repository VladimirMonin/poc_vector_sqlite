````markdown
# 🗺️ Phase 8: Semantic CLI & Control Plane

**Статус:** 🔲 Планируется  
**Зависимости:** Phase 7.0 (Logging Core) ✅

---

## 🎯 Общая цель

Создать профессиональный CLI (`semantic`) как **Control Plane** библиотеки:
- Управление данными (ingest, search)
- Фоновые процессы (worker, queue)
- Диагностика и конфигурация (doctor, init)
- Интерактивный RAG-чат

---

## 🧠 Философия: Human-Friendly + Machine-Readable

CLI имеет два "лица":

1. **Для человека:** Rich-таблицы, спиннеры, Markdown, эмодзи
2. **Для скриптов:** Флаг `--json` выдаёт чистые данные

---

## 📦 Декомпозиция на субфазы

| Фаза | Название | Команды | Приоритет |
|------|----------|---------|-----------|
| **8.3** | Config & Init | `init`, `config`, `doctor` | 🔴 Высокий (фундамент) |
| **8.0** | Core CLI | `ingest`, `search`, `docs` | 🔴 Высокий |
| **8.1** | Operations | `queue`, `worker` | 🟡 Средний |
| **8.2** | RAG Chat | `chat` (REPL + LLM) | 🟡 Средний |

> **Порядок реализации:** 8.3 → 8.0 → 8.1 → 8.2
> 
> Phase 8.3 идёт первой, так как конфигурация — фундамент для всех остальных команд.

---

## 🔧 Архитектура конфигурации (Phase 8.3)

### Единый SemanticConfig

Все настройки библиотеки объединены в один Pydantic Settings класс:

```python
# semantic_core/config.py
from pydantic_settings import BaseSettings

class SemanticConfig(BaseSettings):
    """Единая конфигурация Semantic Core."""
    
    # Database
    db_path: Path = Path("semantic.db")
    
    # Gemini API
    gemini_api_key: str  # Из GEMINI_API_KEY
    gemini_batch_key: str | None = None  # Опционально
    embedding_model: str = "text-embedding-004"
    embedding_dimension: int = 768
    
    # Media
    media_enabled: bool = True
    media_rpm_limit: int = 15
    
    # Processing
    splitter: Literal["simple", "smart"] = "smart"
    context_strategy: Literal["basic", "hierarchical"] = "hierarchical"
    
    # Search defaults
    search_limit: int = 10
    search_type: Literal["vector", "fts", "hybrid"] = "hybrid"
    
    # Logging (делегирует в LoggingConfig)
    log_level: str = "INFO"
    log_file: Path | None = None
    
    model_config = SettingsConfigDict(
        env_prefix="SEMANTIC_",
        env_file=".env",
        toml_file="semantic.toml",  # Pydantic v2.6+
    )
```

### Приоритет настроек

1. CLI аргументы (`--db-path`, `--log-level`)
2. Environment variables (`SEMANTIC_DB_PATH`, `GEMINI_API_KEY`)
3. `semantic.toml` в текущей директории
4. Default values

### Разделение секретов

- **semantic.toml** — несекретные настройки (paths, limits, features)
- **.env / environment** — только API ключи
- CLI никогда не сохраняет секреты в TOML

---

## 📐 Итоговая структура пакета

```text
semantic_core/cli/
├── __init__.py           # main() entry point
├── app.py                # Typer приложение, глобальные callback'и
├── context.py            # CLIContext (DI контейнер)
├── console.py            # Rich Console singleton
├── commands/             # Группы команд
│   ├── __init__.py
│   ├── ingest.py         # Phase 8.0
│   ├── search.py         # Phase 8.0
│   ├── docs.py           # Phase 8.0
│   ├── queue.py          # Phase 8.1
│   ├── worker.py         # Phase 8.1
│   ├── chat.py           # Phase 8.2
│   ├── init.py           # Phase 8.3
│   ├── config.py         # Phase 8.3
│   └── doctor.py         # Phase 8.3
└── ui/                   # Слой представления
    ├── __init__.py
    ├── renderers.py      # Таблицы, ошибки, success
    └── spinners.py       # Progress, Status
```

---

## 🖥️ Итоговое меню (`semantic --help`)

```text
Usage: semantic [OPTIONS] COMMAND [ARGS]...

  🧠 Semantic Core CLI — Ваш второй мозг в терминале.

Options:
  -d, --db-path PATH   Путь к базе данных.
  -l, --log-level LVL  Уровень логирования.
  -j, --json           Вывод в формате JSON.
  -v, --verbose        Подробный вывод.
  --help               Show this message and exit.

Commands:
  ingest   📥 Загрузка и индексация файлов/папок.
  search   🔍 Семантический поиск по базе.
  docs     📚 Встроенная документация.
  queue    📦 Управление очередью задач.
  worker   👷 Управление фоновым воркером.
  chat     💬 Интерактивный RAG-чат.
  init     ⚙️ Инициализация проекта.
  config   🔧 Просмотр конфигурации.
  doctor   🩺 Диагностика окружения.
```

---

## 🔤 Новые эмодзи для EMOJI_MAP

При реализации Phase 8 добавить в `formatters.py`:

| Паттерн | Эмодзи | Добавляется в |
|---------|--------|---------------|
| `cli` | 🖥️ | Phase 8.0 |
| `worker` | 👷 | Phase 8.1 |
| `chat`, `rag` | 💬 | Phase 8.2 |
| `init` | ⚙️ | Phase 8.3 |
| `config` | 🔧 | Phase 8.3 |
| `doctor`, `diagnostic` | 🩺 | Phase 8.3 |

---

## 📚 Документация (после всех субфаз)

### Архитектурный сериал

| Episode | Тема | После фазы |
|---------|------|------------|
| 39 | CLI Architecture | 8.0 |
| 40 | CLI UX Patterns | 8.0 |
| 41 | Worker Architecture | 8.1 |
| 42 | RAG Pipeline | 8.2 |
| 43 | Chat Interface | 8.2 |
| 44 | Configuration Management | 8.3 |

### README обновления

- Секция "CLI Usage" с примерами
- Секция "Background Processing" (worker, queue)
- Секция "Configuration" (semantic.toml)

---

## 🔗 Детальные планы

1. **[Phase 8.3 — Config & Init](phase_8.3.md)** — конфигурация, диагностика (ПЕРВАЯ!)
2. **[Phase 8.0 — Core CLI](phase_8.0.md)** — ingest, search, docs
3. **[Phase 8.1 — Operations CLI](phase_8.1.md)** — queue, worker
4. **[Phase 8.2 — RAG Chat](phase_8.2.md)** — интерактивный чат

---

## 💡 Исходный брейншторм (архив)

<details>
<summary>Оригинальные идеи из обсуждения с архитектором</summary>

### Философия CLI: "Human Friendly, Machine Readable"

CLI должен иметь два лица:

1. **Для человека:** Красивый вывод, спиннеры, таблицы, Markdown-рендеринг (через `Rich`).
2. **Для скриптов/агентов:** Флаг `--json`, который отключает всю красоту и выдает чистые данные для пайпинга (`| jq`).

### Интеграция с проектами (Django/Flask)

**Сценарий Django:**
В своем `manage.py` или в админке ты можешь вызывать команды CLI программно, или просто использовать CLI как внешний инструмент, указывая путь к той же базе данных.

```bash
# В папке джанго проекта
semantic search "юзеры" --db-path ./db.sqlite3
```

*Примечание:* Для этого нам нужно будет научить `semantic_core` понимать, как подключаться к базе, если она занята Джангой (WAL режим SQLite это позволяет).

### Command as a Service Pattern

Каждый файл в `commands/` создает свой `typer.Typer()` объект, который потом монтируется в главное приложение. Это позволяет легко добавить команду `semantic plugins install ...` или `semantic stats` в будущем.

</details>

````
