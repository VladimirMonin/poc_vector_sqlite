---
title: "Гибридный поиск и RRF"
description: "Как объединить векторный и полнотекстовый поиск через Reciprocal Rank Fusion"
...
```plantuml
@startuml
title Гибридный поиск с RRF

start
:Получить запрос "срочный скрипт";

fork
    :Векторный поиск;
    note right: embed_query() → cosine distance
    :TOP 100 по расстоянию;
fork again
    :FTS5 поиск;
    note right: MATCH токенизация
    :TOP 100 по BM25;
end fork
...
note right
  ...
  Документ в обоих → высокий score
end note
...
legend right
  ...
  Меньше k → сильнее влияет ранг
  Больше k → равномернее веса
endlegend
@enduml
```
...
## Понимание Score: разные шкалы 📏

**Важно**: Каждый тип поиска использует свою шкалу Score!

| Тип поиска | Пример Score | Шкала | Интерпретация |
|------------|--------------|-------|---------------|
| **Vector** | 0.713 | 0.0 — 1.0 | Косинусное сходство. 1.0 = идеальное совпадение |
| **FTS** | 6.112 | 0 — ∞ | BM25 ранг. Чем выше — тем релевантнее |
| **Hybrid** | 0.033 | 0.0 — ~0.033 | RRF score. Максимум ≈ 1/(60+1) + 1/(60+1) |

### Почему такие разные числа?

- **Vector 0.71** — документ на 71% семантически похож на запрос
- **FTS 6.1** — BM25 алгоритм, абсолютное значение зависит от длины документа и частоты слов
- **Hybrid 0.033** — формула RRF: `1/(k+rank)`, где k=60. Первое место в обоих поисках даёт ~0.033

### Как сравнивать результаты?

❌ **Нельзя** сравнивать Score между разными типами поиска  
✅ **Можно** сравнивать Score внутри одного поиска (выше = лучше)
